...
bool FVecs::load_from_fvecs_file_fast(const std::string& file_path) {
    ...
    if (!file.is_open()) {
        std::cerr << "Error (Fast Load): Unable to Open the file " << file_path << '\n';
        return false;
    }

    // 1. Determine the file size and Dimension of each vector(input file is expected to consist of
    // a set of fixed dimensional vectors)
    ...
    if (this->header.dim == 0 || this->header.dim < 0) {
        std::cerr << "Error (Fast Load): Invalid Dimension read " << dim << '\n';
        return false;
    }
    ...
    if (file_size < file_record_size) {
        std::cerr << "Error (Fast Load): File size too small for one vector. File truncated."
                  << '\n';
        return false;
    }
    ...
    if (!file.read(buffer.data(), file_size)) {
        std::cerr << "Error (Fast Load): Failed to read file content into memory." << '\n';
        return false;
    }
    ...
    for (size_t i = 0; i < total_num_vectors; ++i) {
        ...
        if (d != dim) {
            std::cerr << "Warning (Fast Load): Dimension mismatch at record " << i << "(" << d
                      << "!=" << dim << ")" << '\n';
            return false;
        }
        buf_ptr += sizeof(int);
        ...
        buf_ptr += dim * sizeof(float);  // move the buffer pointer to the base address of the next
                                         // record/vector in the file
    ...
    if (points.empty()) {
        std::cerr << "No data loaded from " << file_path << '\n';
        return false;
    }
...
bool FVecs::load_first_n_from_fvecs_file_fast(size_t n, const std::string& file_path) {
    std::ifstream file(file_path, std::ios::binary | std::ios::ate);
    if (!file.is_open()) {
        std::cerr << "Error (Fast Load): Unable to open file " << file_path << '\n';
        return false;
    }
    ...
    int dim = 0;
    if (!file.read(reinterpret_cast<char*>(&dim), sizeof(int))) {
        std::cerr << "Error (Fast Load): Unable to read dimension from " << file_path << '\n';
        return false;
    }
    this->header.dim = dim;
    if (dim <= 0) {
        std::cerr << "Error (Fast Load): Invalid dimension " << dim << '\n';
        return false;
    }

    const size_t record_size = sizeof(int) + dim * sizeof(float);
    if (file_size < record_size) {
        std::cerr << "Error (Fast Load): File too small for a single vector." << '\n';
        return false;
    }
    ...
    if (points.empty()) {
        std::cerr << "Error (Fast Load): No vectors loaded from " << file_path << '\n';
        return false;
    }

    std::cout << "Read " << points.size() << " vectors from " << file_path << " successfully."
              << '\n';
    return true;
}
...
bool FVecs::load_from_binary(const std::string& file_path) {
    ...
    if (!file.read(reinterpret_cast<char*>(&num_points_ui), sizeof(unsigned int))) {
        std::cerr << "Error: Failed to read num_points header from binary." << '\n';
        return false;
    }
    if (!file.read(reinterpret_cast<char*>(&dim_ui), sizeof(unsigned int))) {
        std::cerr << "Error: Failed to read dimension header from binary." << '\n';
        return false;
    }
    ...
    if (!file || num_points == 0 || dim == 0) {
        std::cerr << "Error: Invalid or Corrupted Binary (N=" << num_points << ", D=" << dim << ")"
                  << '\n';
        return false;
    }
    points.clear();
    ...
    for (unsigned int i = 0; i < num_points; ++i) {
        ...
        file.read(reinterpret_cast<char*>(point.values.data()), dim * sizeof(float));
        if (!file) {
            std::cerr << "Error: Unexpected EOF while reading the point " << i << '\n';
            return false;
        }
...
// timer.h (simple no-op timer placeholders if you need to keep prints)
struct CPUTimer {
    double start = 0;
    void   Start() { start = 0; }
    void   Stop() {}
...
constexpr uint rg_bin_size_g = 10000;  // initial randomgraph size
...
// each reverse-entry: <count:uint> + max_reverse_index_entries_g uint entries
...
// --- FIX: Add device access to constants used for static array sizes in kernels ---
namespace FreshVamana {
...
// ------------------------------- globals (merged & fixed) -----------------------
namespace FreshVamana {
namespace Globals {
__device__ __managed__ uint d_graph_capacity_g = 0;
__device__ __managed__ uint d_graph_size_g     = 0;

uint*                         d_delete_list_g = nullptr;  // device pointer for delete list (uint*)
FreshVamana::Consts::dtype_g* d_insert_list_g =
    nullptr;  // device pointer for insert list (dtype array)
...
// ------------------------------- bloom_filter (merged) --------------------------
// NOTE: Kept 'using namespace FreshVamana::Consts;' here for BF_ENTRIES
...
// Helper host function that runs the device kernel to check a node id
__host__ inline bool isNodeInDeleteList(uint* d_delete_list, size_t size, uint node_id) {
    ...
    const uint blocks  = (uint)((size + threads - 1) / threads);
    checkIfNodeDeletedKernel<<<blocks, threads>>>(d_delete_list, (uint)size, node_id, d_found);
...
__device__ inline bool isNodeInDeleteList_dev(const uint* d_delete_list,
                                              size_t      size,
                                              uint        node_id) {
...
class InsertList {
   public:
    InsertList(uint initial_capacity = 5000u) {
        ...
        size_          = 0;
        gpuErrchk(
            cudaMalloc(&FreshVamana::Globals::d_insert_list_g, capacity_ * insert_entry_bytes_g));
    }
    ~InsertList() {
    ...
    void addVectors(FreshVamana::Consts::dtype_g* d_vectors, size_t num_vectors) {
        ...
        FreshVamana::Consts::dtype_g* dst = FreshVamana::Globals::d_insert_list_g +
                                            static_cast<size_t>(size_) * FreshVamana::Consts::D_g;
...
// ------------------------------- graph init util (merged) -----------------------

template <typename T__>
[[nodiscard]] inline std::unique_ptr<uint8_t[]> readBinToHost(
    const std::filesystem::path& graph_bin_path, size_t number_of_entries, size_t entry_size) {
    FILE* bin_file = fopen(graph_bin_path.c_str(), "rb");
    if (!bin_file) {
        fprintf(stderr, "Failed to open bin file %s\n", graph_bin_path.c_str());
        return nullptr;
    }
    ...
    fclose(bin_file);
    if (r != read_count) {
        fprintf(stderr, "Incomplete read: %zu of %zu\n", r, read_count);
        return nullptr;
    }
    return buffer;
}
...
template <typename T__>
[[nodiscard]] inline std::unique_ptr<struct GraphT> initGraph_simple(
...
// The initGraph function used in your main
template <typename T__>
[[nodiscard]] inline std::unique_ptr<GraphT> initGraph(
    const std::filesystem::path& graph_bin_path) {
    using namespace FreshVamana::Consts;
    auto graph = std::make_unique<GraphT>();

    FreshVamana::Globals::d_graph_capacity_g = rg_bin_size_g * 13 / 10;  // 1.3x
    ...
    std::unique_ptr<uint8_t[]> readbuf =
        readBinToHost<T__>(graph_bin_path, FreshVamana::Globals::d_graph_size_g, entry_bytes);
    if (!readbuf) {
        free(h_graph);
        return nullptr;
    }
    ...
    // zero full allocation to avoid garbage neighbors / degree
    gpuErrchk(cudaMemset(graph->d_graph, 0, alloc_entries * entry_bytes));
    gpuErrchk(cudaMemcpy(graph->d_graph,
                         h_graph,
                         FreshVamana::Globals::d_graph_size_g * entry_bytes,
                         cudaMemcpyHostToDevice));
    free(h_graph);
    ...
    std::cout << "[ initGraph ] Graph initialized: "
              << "N=" << FreshVamana::Globals::d_graph_size_g << ", D=" << D_g << ", R=" << R_g
              << ", EntrySize=" << entry_bytes << " bytes.\n";

    return graph;
}

// ------------------------------- core kernels (merged & fixed) ------------------

// computeDists (ambiguity fixed by removing using namespace)
template <typename T__>
__global__ void computeDists(uint8_t* d_graph,
                             ...
                             T__*     d_dists,
                             uint     row_size) {
    // FIX: Removed 'using namespace FreshVamana::Consts;' and used qualified names.
    ...
    uint       num_nodes = 0;
    if (d_node_count)
        num_nodes = d_node_count[query_id];
    else
        return;
    ...
    for (uint j = tid / 8; j < num_nodes; j += (blockDim.x + 7) / 8) {
        ...
        // defensive check: node must be < graph size
        if (node >= FreshVamana::Globals::d_graph_size_g)
            continue;
...
// lowerBound/upperBound device helpers
template <typename T__>
...
// sortByDistance (ambiguity fixed by removing using namespace)
template <typename T__>
__global__ void sortByDistance(uint* d_items,
                               ...
                               T__*  d_dists_aux,
                               uint  row_size) {
    // FIX: Removed 'using namespace FreshVamana::Consts;'
    uint query_id = blockIdx.x;
    uint tid      = threadIdx.x;
    uint numItems = d_item_count[query_id];
    uint offset   = query_id * row_size;

    extern __shared__ uint shared_pos[];  // expected size >= blockDim.x, caller must provide

    if (numItems == 0)
        return;

    // Simple parallel merge-sort like operation used in original; keep but guard indices
    for (uint subarray_size = 2; subarray_size < 2 * numItems; subarray_size *= 2) {
        ...
        if (tid >= start && tid < mid) {
            ...
            if (mid < end) {
                before =
                    lowerBound<T__>(&d_dists[offset + mid], 0, end - mid, d_dists[offset + tid]);
                shared_pos[tid - start] = tid + before;
            } else {
                shared_pos[tid - start] = tid;
            }
        } else if (tid >= mid && tid < end) {
            // upper half
            before =
                upperBound<T__>(&d_dists[offset + start], 0, mid - start, d_dists[offset + tid]);
            shared_pos[tid - start] = before + (tid - mid + start);
        }

        __syncthreads();
        __threadfence_block();

        // copy to aux if within bounds
        for (uint i = tid; i < numItems; i += blockDim.x) {
            uint pos = shared_pos[i];
            if (pos < row_size) {
                d_items_aux[offset + pos] = d_items[offset + i];
                d_dists_aux[offset + pos] = d_dists[offset + i];
            }
        }
        __syncthreads();
        __threadfence_block();
...
// getNeighbors - FIX: Pointer scoping fixed. Removed 'using namespace FreshVamana::Consts;'
template <typename T__>
__global__ void getNeighbors(uint8_t* d_graph,
    ...
    // FIX: Define pointers early in the function scope
    uint* degreePtr   = reinterpret_cast<uint*>(d_graph + extendedQueryID * graph_entry_bytes_g +
                                              D_g * sizeof(T__));
    ...
    for (uint ii = tid; ii < degree; ii += blockDim.x) {
        d_neighbors[(R_g + 1) * queryID + ii] = neighborPtr[ii];
    }
}

// mergeIntoVisitedSet (ambiguity fixed by removing using namespace)
template <typename T__>
__global__ void mergeIntoVisitedSet(uint* d_visited_set_count,
    ...
    uint new_visited_set_size = (num_neighbors + visited_set_size > max_num_parents_per_query)
                                    ? max_num_parents_per_query
                                    : num_neighbors + visited_set_size;
    ...
    if (tid < visited_set_size) {
        ...
        new_pos     = before + tid;
    } else if (tid >= max_num_parents_per_query &&
               tid < max_num_parents_per_query + num_neighbors) {
...
// pruneOutNeighbors (ambiguity fixed by removing using namespace)
typedef enum : uint8_t { NODE_INIT = 0, NODE_PRUNED = 1, NODE_NEIGHBOR = 2 } NodeStateLocal;

// NOTE: d_reverse_edge_index is uint* (word-count + entries)
template <typename T__>
__global__ void pruneOutNeighbors(uint8_t*        d_graph,
                                  ...
                                  T__*            d_visited_set_dists,
                                  NodeStateLocal* d_visited_set_status,
                                  T__*            d_query_vecs,
                                  uint*           d_reverse_edge_index,  // CHANGED to uint*
                                  float           alpha) {
    // FIX: Removed 'using namespace FreshVamana::Consts;'
    const uint D_g                         = FreshVamana::Consts::D_g;
    const uint R_g                         = FreshVamana::Consts::R_g;
    ...
    for (uint iter = 1;; ++iter) {
        ...
        if (extended_query_id >= FreshVamana::Globals::d_graph_size_g)
            return;

        uint* degree_ptr = reinterpret_cast<uint*>(
            d_graph + extended_query_id * graph_entry_bytes_g + D_g * sizeof(T__));
        uint* neighborPtr = degree_ptr + 1;

        if (tid == 0 && iter == 1) {
            for (uint i = 0; i < num_nodes && i < max_num_parents_per_query; ++i) {
                d_visited_set_status[visited_set_offset + i] = NODE_INIT;
            }
            // set degree to zero (safe)
            atomicExch(degree_ptr, 0u);
        }

        __syncthreads();

        // Don't add more neighbors if R reached
        if (*degree_ptr >= R_g)
            return;
        ...
        // Find p_star (serial on thread 0)
        if (tid == 0) {
            for (uint i = 0; i < num_nodes && i < max_num_parents_per_query; ++i) {
                if (d_visited_set_status[visited_set_offset + i] != NODE_INIT)
                    continue;
                p_star_shared[0] = d_visited_set[visited_set_offset + i];

                // add edge from query to p_star
                uint old_degree = atomicAdd(degree_ptr, 1u);
                if (old_degree < R_g) {
                    neighborPtr[old_degree] = p_star_shared[0];
                } else {
                    // exceed R_g: roll back the increment
                ...
                // Add reverse edge entry safely into uint-array per-node
                uint node_idx = p_star_shared[0];
                if (node_idx < FreshVamana::Globals::d_graph_size_g) {
                    uint* entryPtr = d_reverse_edge_index +
                                     node_idx * reverse_index_entry_words_g;  // first word is count
                    uint oldLen = atomicAdd(&entryPtr[0], 1u);
                    if (oldLen < max_reverse_index_entries_g) {
                        entryPtr[1 + oldLen] = extended_query_id;
                    } else {
                        // Hit reverse-index limit: undo count bump
        ...
        uint pStar = p_star_shared[0];
        if (pStar == UINT_MAX)
            return;

        // copy pStar vector into shared
        __shared__ T__ pStarVec[D_g];
        const T__*     vecPtr = reinterpret_cast<const T__*>(d_graph + pStar * graph_entry_bytes_g);
        for (uint ii = tid; ii < D_g; ii += blockDim.x) {
            pStarVec[ii] = vecPtr[ii];
        }
        __syncthreads();

        // warp cooperative reduction as before
        ...
        for (uint ii = warpId; ii < num_nodes; ii += warpsPerBlock) {
            if (ii >= max_num_parents_per_query)
                continue;
            if (d_visited_set_status[visited_set_offset + ii] != NODE_INIT)
                continue;

            uint p = d_visited_set[visited_set_offset + ii];
            if (p >= FreshVamana::Globals::d_graph_size_g)
                continue;
        ...
        // loop - the kernel will iterate until no p_star found or degree >= R_g
    }
}

// loadQueryVecs (ambiguity fixed by removing using namespace)
template <typename T__>
__global__ void loadQueryVecs(uint8_t* d_graph, T__* d_queryVecs) {
    ...
    uint query_id = blockIdx.x;
    uint tid      = threadIdx.x;
    if (query_id >= FreshVamana::Globals::d_graph_size_g)
        return;
...
// parseReverseIndex (ambiguity fixed by using DeviceConsts)
__global__ void parseReverseIndex(uint* d_reverseEdgeIndex,
                                  uint* d_reverseEdges,
                                  uint* d_reverseEdgeCount,
                                  uint* degreeCounts) {
    // FIX: Use FreshVamana::DeviceConsts:: for array sizes (to fix ambiguity)
    ...
    uint* entryPtr =
        d_reverseEdgeIndex + queryID * FreshVamana::DeviceConsts::reverse_index_entry_words_g;
    ...
    for (uint ii = tid; ii < numReverseEdges; ii += blockDim.x) {
        if (ii < FreshVamana::DeviceConsts::max_reverse_index_entries_g)
            d_reverseEdges[queryID * FreshVamana::DeviceConsts::max_reverse_index_entries_g + ii] =
                entryPtr[1 + ii];
    }
}

// getPrunableQueryIDs (ambiguity fixed by removing using namespace)
...
// mergeIntoReverseEdges (ambiguity fixed by removing using namespace)
template <typename T__>
__global__ void mergeIntoReverseEdges(uint* d_reverseEdgeCount,
    ...
    if (tid < reverseEdgeCount) {
        ...
        newPos      = before + tid;
    } else if (tid >= max_reverse_index_entries_g &&
               tid < max_reverse_index_entries_g + numNeighbors) {
...
// pruneReverseEdges (ambiguity fixed by removing using namespace)
template <typename T__>
__global__ void pruneReverseEdges(uint8_t*        d_graph,
                                  ...
                                  uint*           d_reverseEdgeCount,
                                  T__*            d_reverseEdgeDists,
                                  NodeStateLocal* d_reverseEdgeStatus,
                                  T__*            d_queryVecs,
                                  float           alpha) {
    // FIX: Removed 'using namespace FreshVamana::Consts;'
    const uint D_g                         = FreshVamana::Consts::D_g;
    ...
    uint* degreePtr =
        reinterpret_cast<uint*>(d_graph + queryID * graph_entry_bytes_g + D_g * sizeof(T__));
    uint* neighborPtr = degreePtr + 1;

    if (tid == 0) {
        for (uint i = 0; i < numNodes && i < max_reverse_index_entries_g; ++i)
            d_reverseEdgeStatus[reverseEdgeOffset + i] = NODE_INIT;
    ...
    if (tid == 0) {
        for (uint i = 0; i < numNodes && i < max_reverse_index_entries_g; ++i) {
            if (d_reverseEdgeStatus[reverseEdgeOffset + i] != NODE_INIT)
                continue;
    ...
    for (uint ii = warpId; ii < numNodes; ii += warpsPerBlock) {
        if (ii >= max_reverse_index_entries_g)
            continue;
        if (d_reverseEdgeStatus[reverseEdgeOffset + ii] != NODE_INIT)
            continue;
...
// findPointsKernel (ambiguity fixed by removing using namespace)
template <typename T__>
...
// copyNewVectorsToGraph (ambiguity fixed by removing using namespace)
template <typename T__>
__global__ void copyNewVectorsToGraph(uint8_t*   d_graph,
    ...
    if (graph_node_id >= FreshVamana::Globals::d_graph_capacity_g)
        return;  // defensive
    ...
    if (tid == 0) {
        uint* degree_ptr =
            reinterpret_cast<uint*>(reinterpret_cast<char*>(dest_vec_ptr) + D_g * sizeof(T__));
        *degree_ptr = 0u;
        // set neighbours to UINT_MAX
        uint* nbptr = degree_ptr + 1;
        for (uint k = 0; k < R_g; ++k)
            nbptr[k] = UINT_MAX;
    }
}

// ------------------------------- host-side computeReverseEdges & OutNeighbors (fixed)
// -----------------------

// Forward declaration of greedySearch so Vamana (below) can call it even though the implementation
// is later in file.
template <typename T__>
uint* greedySearch(uint8_t*    d_graph,
...
void computeOutNeighbors(uint8_t* d_graph,
    ...
    NodeStateLocal* d_visitedSetStatus;

    gpuErrchk(cudaMalloc(&d_visitedSetDists,
                         (size_t)batch_size * Consts::max_num_parents_per_query * sizeof(T__)));
    gpuErrchk(cudaMalloc(&d_visitedSetAux,
                         (size_t)batch_size * Consts::max_num_parents_per_query * sizeof(uint)));
    gpuErrchk(cudaMalloc(&d_visitedSetDistsAux,
                         (size_t)batch_size * Consts::max_num_parents_per_query * sizeof(T__)));
    gpuErrchk(cudaMalloc(
        &d_visitedSetStatus,
        (size_t)batch_size * Consts::max_num_parents_per_query * sizeof(NodeStateLocal)));
    ...
    // Run pipeline with safer kernels + synchronized checks
    getNeighbors<T__>
        <<<batch_size, Consts::R_g>>>(d_graph, batch_start, d_neighbors, d_neighborsCount);
    gpuErrchk(cudaPeekAtLastError());
    gpuErrchk(cudaDeviceSynchronize());

    computeDists<T__><<<batch_size, Consts::R_g * 8>>>(
        d_graph, d_neighbors, d_neighborsCount, d_query_vecs, d_neighborsDists, (Consts::R_g + 1));
    gpuErrchk(cudaPeekAtLastError());
    gpuErrchk(cudaDeviceSynchronize());
...
// computeReverseEdges (host) - now uses uint* reverse index and includes bounds checks
template <typename T__>
void computeReverseEdges(uint8_t* d_graph, uint* d_reverseEdgeIndex, float alpha) {
    ...
    gpuErrchk(cudaMalloc(
        &d_reverseEdges,
        FreshVamana::Globals::d_graph_size_g * max_reverse_index_entries_g * sizeof(uint)));
    gpuErrchk(cudaMalloc(&d_reverseEdgeCount, FreshVamana::Globals::d_graph_size_g * sizeof(uint)));
    gpuErrchk(cudaMalloc(
        &d_reverseEdgeDists,
        FreshVamana::Globals::d_graph_size_g * max_reverse_index_entries_g * sizeof(T__)));
    gpuErrchk(cudaMalloc(
        &d_reverseEdgesAux,
        FreshVamana::Globals::d_graph_size_g * max_reverse_index_entries_g * sizeof(uint)));
    gpuErrchk(cudaMalloc(
    ...
    gpuErrchk(cudaMalloc(&d_reverseEdgeStatus,
                         FreshVamana::Globals::d_graph_size_g * max_reverse_index_entries_g *
                             sizeof(NodeStateLocal)));
    ...
    gpuErrchk(
        cudaMalloc(&d_neighbors, FreshVamana::Globals::d_graph_size_g * (R_g + 1) * sizeof(uint)));
    gpuErrchk(cudaMalloc(&d_neighborsCount, FreshVamana::Globals::d_graph_size_g * sizeof(uint)));
    gpuErrchk(cudaMalloc(&d_neighborDists,
                         FreshVamana::Globals::d_graph_size_g * (R_g + 1) * sizeof(T__)));
    gpuErrchk(cudaMalloc(&d_neighborsAux,
                         FreshVamana::Globals::d_graph_size_g * (R_g + 1) * sizeof(uint)));
    ...
    uint* degreeSum;
    gpuErrchk(cudaMalloc(&degreeSum, (max_reverse_index_entries_g + 1) * sizeof(uint)));
    gpuErrchk(cudaMemset(degreeSum, 0, (max_reverse_index_entries_g + 1) * sizeof(uint)));

    // parse reverse index: note d_reverseEdgeIndex is a uint* where each entry length is
    // reverse_index_entry_words_g
    ...
    gpuErrchk(cudaMemset(d_queryCount, 0, sizeof(uint)));
    ...
    // compute distances on reverse edges
    computeDists<T__><<<FreshVamana::Globals::d_graph_size_g, 1024>>>(d_graph,
                                                                      d_reverseEdges,
                                                                      d_reverseEdgeCount,
                                                                      d_queryVecs,
                                                                      d_reverseEdgeDists,
                                                                      max_reverse_index_entries_g);
    gpuErrchk(cudaPeekAtLastError());
    gpuErrchk(cudaDeviceSynchronize());
    ...
    sortByDistance<T__>
        <<<FreshVamana::Globals::d_graph_size_g,
           1024,
           reverse_index_entry_words_g * sizeof(uint)>>>(d_reverseEdges,
                                                         d_reverseEdgeCount,
                                                         d_reverseEdgeDists,
                                                         d_reverseEdgesAux,
                                                         d_reverseEdgeDistsAux,
    ...
    getNeighbors<T__>
        <<<FreshVamana::Globals::d_graph_size_g, R_g>>>(d_graph, 0, d_neighbors, d_neighborsCount);
    gpuErrchk(cudaPeekAtLastError());
    gpuErrchk(cudaDeviceSynchronize());

    // compute neighbor distances
    computeDists<T__><<<FreshVamana::Globals::d_graph_size_g, R_g * 8>>>(
    ...
    // sort neighbor lists
    sortByDistance<T__>
        <<<FreshVamana::Globals::d_graph_size_g, (R_g + 1), (R_g + 1) * sizeof(uint)>>>(
            d_neighbors,
            d_neighborsCount,
            d_neighborDists,
            d_neighborsAux,
    ...
    // merge into reverse edges
    mergeIntoReverseEdges<T__><<<FreshVamana::Globals::d_graph_size_g, 1024>>>(d_reverseEdgeCount,
                                                                               d_reverseEdges,
                                                                               d_reverseEdgeDists,
                                                                               d_neighborsCount,
                                                                               d_neighbors,
    ...
    // prune reverse edges: launch for full graph size - safe because kernel checks limits
    pruneReverseEdges<T__><<<FreshVamana::Globals::d_graph_size_g, 32>>>(d_graph,
                                                                         d_queryIDs,
                                                                         d_reverseEdges,
                                                                         d_reverseEdgeCount,
                                                                         d_reverseEdgeDists,
    ...
    gpuErrchk(cudaFree(degreeSum));
    gpuErrchk(cudaFree(d_queryIDs));
    gpuErrchk(cudaFree(d_queryCount));
}

// ------------------------------- search & merge kernels (extraction/merge) ---------------------
...
__global__ void extract_vectors_kernel(const uint8_t* d_graph,
    ...
    if (node_id >= FreshVamana::Globals::d_graph_size_g)
        return;
...
__global__ void merge_and_rerank_kernel(const T__*   d_queryVecs,
                                        ...
                                        const size_t L_g_param,  // Run-time value of L_g
                                        const size_t D_g_param,  // Run-time value of D_g
                                        const size_t insert_list_size,
                                        const size_t num_queries) {
    // FIX: Use DeviceConsts for compile-time sizing (to fix ambiguity)
    using namespace FreshVamana::DeviceConsts;
    ...
    if (t_idx == 0) {
        ...
        T__* g_top_vectors = d_final_top_vectors + q_idx * FreshVamana::DeviceConsts::L_g *
                                                       FreshVamana::DeviceConsts::D_g;
        float l_top_dists[FreshVamana::DeviceConsts::L_g];
        for (size_t k = 0; k < FreshVamana::DeviceConsts::L_g; ++k)
            l_top_dists[k] = 1e30f;  // large device-friendly constant

        // Use the run-time parameters for loops
        const size_t L_g_run = L_g_param;
        const size_t D_g_run = D_g_param;

        // consider worklist vectors
        for (size_t c_idx = 0; c_idx < L_g_run; ++c_idx) {
            ...
            for (size_t d = 0; d < D_g_run; ++d) {
                float diff = static_cast<float>(d_queryVecs[q_idx * D_g_run + d]) -
                             static_cast<float>(cand_vec_ptr[d]);
                dist += diff * diff;
            }
            float  max_dist = -1.0f;
            size_t max_k    = 0;
            for (size_t k = 0; k < L_g_run; ++k)
...
// ------------------------------- Vamana class + host glue (merged & fixed) -----------------------
...
class Vamana {
   public:
    ...
    void deletePoints(T__* d_query_vecs, size_t num_d_query_vecs) {
        CPUTimer cputimer;
        cputimer.Start();
        int* d_results = findPointsInGraph(graph_->d_graph,
                                           FreshVamana::Globals::d_graph_size_g,
                                           d_query_vecs,
                                           (uint)num_d_query_vecs);
        cputimer.Stop();
        // convert results into delete_list
    ...
    [[nodiscard]] uint* searchPoints(T__* d_queryVecs, size_t num) {
        ...
        gpuErrchk(
            cudaMalloc(&d_visitedSetCount, FreshVamana::Globals::d_graph_size_g * sizeof(uint)));
        // allocate reverse index as uint* (one uint word count + entries)
        gpuErrchk(cudaMalloc(&d_reverseEdgeIndex_uint,
                             FreshVamana::Globals::d_graph_size_g *
                                 FreshVamana::Consts::reverse_index_entry_words_g * sizeof(uint)));
        gpuErrchk(
            cudaMemset(d_visitedSetCount, 0, FreshVamana::Globals::d_graph_size_g * sizeof(uint)));
        printf("vamanaInner mallocs: (searchPoints) done\n");
    ...
    [[nodiscard]] T__* searchPoints2(T__* d_queryVecs, size_t num) {
        ...
        // Pass D_g and L_g as parameters
    ...
    void saveGraph(const std::string& output_path) {
        ...
        if (written != total_bytes) {
            fprintf(stderr,
                    "[saveGraph] WARNING: Incomplete write! %zu of %zu bytes written.\n",
                    written,
                    total_bytes);
        } else {
    ...
    void patchGraph() {
        ...
        if (old_graph_size > 0) {
            ...
            gpuErrchk(cudaMemcpy(
                d_graph_new, graph_->d_graph, old_graph_bytes, cudaMemcpyDeviceToDevice));
        }
        ...
        copyNewVectorsToGraph<T__>
            <<<grid, block>>>(d_graph_new, d_new_vecs, (uint)old_graph_size, (uint)num_new_nodes);
        gpuErrchk(cudaPeekAtLastError());
        gpuErrchk(cudaDeviceSynchronize());

        // free old graph and update pointers & sizes
        cudaFree(graph_->d_graph);
        graph_->d_graph         = d_graph_new;
        Globals::d_graph_size_g = static_cast<uint>(new_graph_size);

        // allocate visited sets for new nodes only
        uint* d_visitedSets;
        uint* d_visitedSetCount;
        gpuErrchk(
            cudaMalloc(&d_visitedSets,
                       (size_t)num_new_nodes * Consts::max_num_parents_per_query * sizeof(uint)));
        ...
        // allocate reverse index as uint* sized to full graph size
        uint* d_reverseEdgeIndex;
        gpuErrchk(cudaMalloc(
            &d_reverseEdgeIndex,
            (size_t)Globals::d_graph_size_g * Consts::reverse_index_entry_words_g * sizeof(uint)));
...
// ------------------------------- greedy search and supporting kernels (merged)
// -----------------------
...
__global__ void initializeWorklist(uint8_t* d_graph,
    ...
    const T__* medoidVec = reinterpret_cast<const T__*>(
        d_graph + FreshVamana::Consts::graph_entry_bytes_g * FreshVamana::Consts::medoid_g);
...
__global__ void filterNeighbors(uint8_t*    d_graph,
    ...
    // degree ptr and neighbor ptr are in parent's entry
    ...
    uint* degreePtr =
        reinterpret_cast<uint*>(d_graph + parent * FreshVamana::Consts::graph_entry_bytes_g +
    ...
    uint degree = *degreePtr;
    if (degree > FreshVamana::Consts::R_g)
        degree = FreshVamana::Consts::R_g;

    for (uint ii = tid; ii < degree; ii += blockDim.x) {
        uint neighbor = neighborPtr[ii];

        if (d_delete_list_size &&
            isNodeInDeleteList_dev(d_delete_list, (uint)d_delete_list_size, neighbor))
            continue;
        if (neighbor == queryID)
            continue;
...
// mergeIntoWorklist - with bounds guards
template <typename T__>
__global__ void mergeIntoWorklist(uint* d_worklistCount,
    ...
    extern __shared__ uint sortedPositions[];  // requires launch with proper size
    ...
    if (newPos < newWorklistSize && newPos < FreshVamana::Consts::L_g) {
        ...
        d_worklistVisited[worklistOffset + newPos] =
            (tid < worklistSize) ? d_worklistVisited[worklistOffset + tid] : false;
    }
...
// greedySearch host function - identical flow but with safety checks and error checks
template <typename T__>
[[nodiscard]] uint* greedySearch(uint8_t*    d_graph,
    ...
    do {
        ...
        // filter neighbors
        filterNeighbors<T__><<<batchSize, R_g>>>(d_graph,
                                                 d_delete_list,
                                                 d_delete_list_size,
                                                 d_hasParent,
                                                 d_parents,
        ...
        sortByDistance<T__><<<batchSize, R_g, R_g * sizeof(uint)>>>(d_neighbors,
                                                                    d_neighborsCount,
                                                                    d_neighborDists,
                                                                    d_neighborsAux,
                                                                    d_neighborDistsAux,
        ...
        mergeIntoWorklist<T__>
            <<<batchSize, R_g + L_g, (R_g + L_g) * sizeof(uint)>>>(d_worklistCount,
                                                                   d_worklist,
                                                                   d_worklistDist,
                                                                   d_worklistVisited,
                                                                   d_neighborsCount,
...
class WorkloadGenerator {
   public:
    ...
    void loadBaseVectors(const std::string& path) {
        ...
        if (fvecs_data.header.dim != D) {
            throw std::runtime_error("Dimension mismatch in basepoints file. Expected " +
                                     std::to_string(D) + ", got " +
                                     std::to_string(fvecs_data.header.dim));
        }
    ...
    void loadBaseVectorsFromFvecs(const std::string& fvecs_path, const std::string& temp_bin_path) {
        ...
        if (fvecs_data.header.dim != D) {
            throw std::runtime_error("Dimension mismatch in FVECS file. Expected " +
                                     std::to_string(D) + ", got " +
                                     std::to_string(fvecs_data.header.dim));
        }
        ...
        if (!fvecs_data.save_to_binary(temp_bin_path)) {
            throw std::runtime_error(
                "Failed to convert and save FVECS data to temporary BIN file.");
        }
   ...
   private:
    ...
    std::uniform_int_distribution<size_t> randomBaseIndex_ =
    ...
    Query makeDelete() {
        ...
        // Remove the chosen vector from the active set (to ensure a valid deletion target isn't
        // used again)
...
int main(int argc, char** argv) {
    ...
    if (!graph) {
        fprintf(stderr, "Failed to init graph\n");
        return 1;
    }

    std::cout << "Loaded graph: capacity=" << FreshVamana::Globals::d_graph_capacity_g
              << " size=" << FreshVamana::Globals::d_graph_size_g << "\n";

    // ----------------------------------------------------------------------
    // 2. Build Vamana index
    // ----------------------------------------------------------------------
    ...
    // ----------------------------------------------------------------------
    WorkloadGenerator<float> gen(FreshVamana::Consts::D_g,
                                 0.30f,  // 30% insert
                                 0.10f,  // 10% delete
                                 0.60f   // 60% search
    );
    ...
    for (int iter = 0; iter < NUM_ITER; iter++) {
        ...
        // -----------------------------------------
        // Perform operations on Vamana index
        // -----------------------------------------
        if (nS > 0) {
            auto* ddd = index.searchPoints2(d_search, nS);
            cudaFree(ddd);
        }

        if (nI > 0) {
            index.insertPoints(d_insert, nI);
        }

        if (nD > 0) {
            index.deletePoints(d_delete, nD);
        }

        // Release temporary GPU buffers
        if (d_search)
            cudaFree(d_search);
        if (d_insert)
            cudaFree(d_insert);
        ...
        printf("[Main] Completed workload iteration %d\n", iter);
    }
