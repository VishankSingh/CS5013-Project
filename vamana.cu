...
FreshVamana::Consts::dtype_g* d_insert_list_g = nullptr; // device pointer for insert list (dtype array)
...
__device__ inline bool isNodeInDeleteList_dev(const uint* d_delete_list, size_t size, uint node_id) {
    for (size_t i = 0; i < size; ++i) {
        if (d_delete_list[i] == node_id) return true;
    }
    return false;
}
  ...
  public:
    ...
    void addVectors(FreshVamana::Consts::dtype_g* d_vectors, size_t num_vectors) {
        ...
        FreshVamana::Consts::dtype_g* dst =
            FreshVamana::Globals::d_insert_list_g + static_cast<size_t>(size_) * FreshVamana::Consts::D_g;
...
[[nodiscard]] inline std::unique_ptr<uint8_t[]> readBinToHost(const std::filesystem::path& graph_bin_path,
...
template <typename T__>
[[nodiscard]] inline std::unique_ptr<struct GraphT> initGraph_simple(const std::filesystem::path& graph_bin_path);
...
[[nodiscard]] inline std::unique_ptr<GraphT> initGraph(const std::filesystem::path& graph_bin_path) {
    ...
    std::cout << "[ initGraph ] Graph initialized: "
              << "N=" << FreshVamana::Globals::d_graph_size_g
              << ", D=" << D_g
              << ", R=" << R_g
...
__global__ void computeDists(uint8_t* d_graph,
    ...
    uint num_nodes = 0;
    if (d_node_count) num_nodes = d_node_count[query_id];
    else return;
...
__global__ void sortByDistance(uint* d_items,
    ...
    for (uint subarray_size = 2; subarray_size < 2 * numItems; subarray_size *= 2) {
        ...
        if (tid >= start && tid < mid) {
            ...
            // upper half
            before = upperBound<T__>(&d_dists[offset + start], 0, mid - start, d_dists[offset + tid]);
...
__global__ void pruneOutNeighbors(uint8_t* d_graph,
    ...
    for (uint iter = 1;; ++iter) {
        ...
        if (tid == 0) p_star_shared[0] = UINT_MAX;
        __syncthreads();
        ...
        if (tid == 0) {
            for (uint i = 0; i < num_nodes && i < max_num_parents_per_query; ++i) {
                ...
                if (node_idx < FreshVamana::Globals::d_graph_size_g) {
                    uint* entryPtr = d_reverse_edge_index + node_idx * reverse_index_entry_words_g; // first word is count
...
__global__ void mergeIntoReverseEdges(uint* d_reverseEdgeCount,
    ...
    uint newReverseEdgeCount = std::min(numNeighbors + reverseEdgeCount, max_reverse_index_entries_g);
...
__global__ void copyNewVectorsToGraph(uint8_t* d_graph,
    ...
    if (graph_node_id >= FreshVamana::Globals::d_graph_capacity_g) return; // defensive
...
// Forward declaration of greedySearch so Vamana (below) can call it even though the implementation is later in file.
template <typename T__>
...
void computeOutNeighbors(uint8_t* d_graph,
    ...
    gpuErrchk(cudaMalloc(&d_neighborsDistsAux, (size_t)batch_size * (Consts::R_g + 1) * sizeof(T__)));
...
void computeReverseEdges(uint8_t* d_graph, uint* d_reverseEdgeIndex, float alpha) {
    ...
    // parse reverse index: note d_reverseEdgeIndex is a uint* where each entry length is reverse_index_entry_words_g
...
class Vamana {
  public:
    ...
    [[nodiscard]] uint* searchPoints(T__* d_queryVecs, size_t num) {
        uint* d_visitedSets; uint* d_visitedSetCount; uint* d_reverseEdgeIndex_uint;
        std::cout << "[ searchPoints ]\n";
        CPUTimer cputimer; CPUTimer cputimermain; cputimermain.Start();
    ...
    [[nodiscard]] T__* searchPoints2(T__* d_queryVecs, size_t num) {
        ...
        merge_and_rerank_kernel<<<merge_grid, merge_block>>>(d_queryVecs, d_worklist_vectors, FreshVamana::Globals::d_insert_list_g, d_final_top_vectors, L_g, D_g, insert_list_.size(), num);
        gpuErrchk(cudaPeekAtLastError());
        gpuErrchk(cudaDeviceSynchronize());
  ...
  private:
    ...
    [[nodiscard]] int* findPointsInGraph(const uint8_t* d_graph, uint n_nodes, const T__* d_query_vecs, uint n_queries) {
        ...
        findPointsKernel<T__><<<blocks, threads>>>(d_graph, d_query_vecs, n_nodes, n_queries, d_results);
        cudaDeviceSynchronize();
        return d_results;
    }
    ...
    void runVamana() {
       ...
       // Each graph entry starts at graph_->d_graph + i * graph_entry_bytes_g and its vector is the first D_g elements.
       for (uint i = 0; i < Globals::d_graph_size_g; ++i) {
           // source pointer: device memory for the i-th entry vector (first D_g elements)
           const T__* src = reinterpret_cast<const T__*>(graph_->d_graph + static_cast<size_t>(i) * Consts::graph_entry_bytes_g);
        ...
        gpuErrchk(cudaMalloc(&d_reverseEdgeIndex, Globals::d_graph_size_g * Consts::reverse_index_entry_words_g * sizeof(uint)));
        gpuErrchk(cudaMemset(d_visitedSetCount, 0, Globals::d_graph_size_g * sizeof(uint)));
        gpuErrchk(cudaMemset(d_reverseEdgeIndex, 0, Globals::d_graph_size_g * Consts::reverse_index_entry_words_g * sizeof(uint)));

        uint* d_worklist = greedySearch<T__>(graph_->d_graph, delete_list_.data(), delete_list_.size(), d_queryVecs, d_visitedSets, d_visitedSetCount, Globals::d_graph_size_g);
        gpuErrchk(cudaFree(d_worklist));

        computeOutNeighbors<T__>(graph_->d_graph, d_queryVecs, d_visitedSets, d_visitedSetCount, 1.5f, d_reverseEdgeIndex, 0, Globals::d_graph_size_g);
...
__global__ void mergeIntoWorklist(uint* d_worklistCount,
    ...
    uint newWorklistSize = std::min<uint>(numNeighbors + worklistSize, FreshVamana::DeviceConsts::L_g);
...
[[nodiscard]] uint* greedySearch(uint8_t* d_graph,
    ...
    gpuErrchk(cudaPeekAtLastError());
    initializeWorklist<T__><<<batchSize, 1>>>(d_graph, d_queryVecs, d_worklist, d_worklistCount, d_worklistDist, d_worklistVisited);
    gpuErrchk(cudaPeekAtLastError());
    gpuErrchk(cudaDeviceSynchronize());
    ...
    do {
        ...
        computeDists<T__><<<batchSize, R_g * 8>>>(d_graph, d_neighbors, d_neighborsCount, d_queryVecs, d_neighborDists, (R_g + 1));
        gpuErrchk(cudaPeekAtLastError());
        gpuErrchk(cudaDeviceSynchronize());
...
int main(int argc, char** argv) {
    ...
    std::cout << "Loaded graph: capacity="
              << FreshVamana::Globals::d_graph_capacity_g
    ...
    for (int iter = 0; iter < NUM_ITER; iter++) {
        ...
        // -----------------------------------------
        if (nS > 0) {
            index.searchPoints2(d_search, nS);
        }
